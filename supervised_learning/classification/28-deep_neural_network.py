#!/usr/bin/env python3
"""Deep neural network class (multiclass) with selectable hidden activations"""
...
class DeepNeuralNetwork:
    ...
    @property
    def activation(self):
        """Getter for activation type used in hidden layers"""
        return self.__activation

    def forward_prop(self, X):
        """Calculates forward propagation
        (hidden: sig/tanh, output: softmax)"""
    ...
    def gradient_descent(self, Y, cache, alpha=0.05):
        ...
        m = Y.shape[1]
        dZ = cache["A{}".format(self.__L)] - Y  # softmax + cross-entropy
        ...
        for layer in range(self.__L, 0, -1):
            ...
            if layer > 1:
                # dZ_{l-1} = (W_l^T dZ_l) * g'(A_{l-1})
                if self.__activation == 'sig':
                    deriv = A_prev * (1 - A_prev)
                else:
                    deriv = 1 - (A_prev ** 2)  # tanh'(z) = 1 - tanh(z)^2
